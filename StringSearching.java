...
public class StringSearching {
    ...
    public static List<Integer> kmp(CharSequence pattern, CharSequence text) {
        if (pattern == null || pattern.length() == 0 || text == null) {
            throw new IllegalArgumentException("Input pattern or text is null"
                    + ", please enter valid inputs next time.");
        }
    ...
    public static Map<Character, Integer> buildLastTable(CharSequence pattern) {
        if (pattern == null) {
            throw new IllegalArgumentException("Input pattern is null, please"
    ...
    public static List<Integer> rabinKarp(CharSequence pattern,
        ...
        while (i <= n - m) {
            ...
            i++;
            if (i <= n - m) {
                textHash = updateHash(textHash, m, text.charAt(i - 1),
                        text.charAt(i - 1 + m));
            }
        }
        return matches;
    ...
    public static int generateHash(CharSequence current, int length) {
        if (current == null) {
            throw new IllegalArgumentException("Input character sequence is"
                    + " null, please use a valid input next time.");
        }
        if (length < 0 || length == 0 || length > current.length()) {
            throw new IllegalArgumentException("Length " + length + " is "
                    + "invalid, please use a valid input next time.");
        }
        baseMap.put(0, 1);
        int i = 1;
        while (i < length) {
            baseMap.put(i, BASE * baseMap.get(i - 1));
            i++;
        }
        int j = 0;
        int hash = 0;
        while (j < length) {
            hash += current.charAt(j) * baseMap.get(length - 1 - j);
            j++;
        }
        return hash;
    }
    ...
    public static int updateHash(int oldHash, int length, char oldChar,
        ...
        return (oldHash - oldChar * baseMap.get(length - 1)) * BASE + newChar;
    }
}
